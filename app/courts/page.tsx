    ...
    <div className="court-selection-grid">
      {courts.map((court) => {
        ...
        return <article className="selection-card" key={court.id}>
          ...
          <div className="selection-game"><small>{court.currentGame?.status === "AWAITING_SCORE" ? copy.awaitingScore : copy.nowPlaying}</small><strong>{court.currentGame ? <>{court.currentGame.home} <b>VS</b> {court.currentGame.away}</> : copy.noCurrentGame}</strong></div>
