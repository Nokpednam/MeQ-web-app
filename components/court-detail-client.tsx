...
const statusKeys: Record<QueueEntryStatus, "waiting" | "called" | "readyToPlay" | "playing" | "awaitingScore" | "resting"> = {
  WAITING: "waiting", CALLED: "called", READY_TO_PLAY: "readyToPlay", PLAYING: "playing", AWAITING_SCORE: "awaitingScore", RESTING: "resting",
};
...
export function CourtDetailClient({ courtId }: { courtId: string }) {
  ...
  const queueEntries = queueState.entries.filter((entry) => entry.courtId === selectedCourt.id && entry.status !== "PLAYING" && entry.status !== "AWAITING_SCORE").sort((a, b) => a.position - b.position || a.joinedAt.localeCompare(b.joinedAt));
  ...
  function actionContent() {
    ...
    if (entryOnThisCourt) {
      const canLeave = isCaptain && !["PLAYING", "AWAITING_SCORE", "RESTING"].includes(entryOnThisCourt.status);
