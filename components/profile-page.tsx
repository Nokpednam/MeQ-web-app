...
type ProfileHistory = {
  gameId: string;
  teamType: "THREE_X_THREE" | "FIVE_X_FIVE";
  points: number;
  won: boolean;
  completedAt: string;
};
...
function initialsFor(displayName: string) {
  return Array.from(displayName.trim()).slice(0, 2).join("").toUpperCase() || "NU";
}

export function ProfilePage({ authProfile }: { authProfile: AuthProfile }) {
  ...
  const history = authProfile.history;
  const historyWins = history.filter((item) => item.won).length;
  const games = history.length;
  const wins = historyWins;
  const winRate = history.length ? Math.round(historyWins/history.length*100) : 0;
  ...
  return <div className="app-frame profile-app">
    ...
    <main className="profile-shell">
      ...
      <div className="mock-banner profile-mock" role="note"><span>DEV</span><div><strong>{copy.connected}</strong><small>{copy.localData}</small></div></div>
      <section className="profile-identity"><div className="profile-avatar-large">{authProfile.avatarUrl?<Image alt="" height={112} priority src={authProfile.avatarUrl} width={112}/>:initialsFor(authProfile.displayName)}</div><div><small>{authProfile.role}</small><h2>{authProfile.displayName}</h2><p>{copy.team}: <strong>{authProfile.team?.name??copy.noTeam}</strong></p></div><div className="profile-account-actions">{authProfile.team?<Link href={`/teams/${authProfile.team.id}`}>{copy.manageTeam} →</Link>:<Link href="/teams/create">{dashboardCopy.createTeam} →</Link>}<form action={signOut}><button type="submit">{copy.signOut}</button></form></div></section>
      ...
      <section className="profile-history"><div className="section-heading"><div><p className="section-label">HISTORY</p><h2>{copy.recent}</h2></div></div>{history.length?<ol>{history.slice(0,5).map((item)=><li key={item.gameId}><Link href={`/games/${item.gameId}/result`}><span className={item.won?"is-win":"is-loss"}>{item.won?copy.won:copy.lost}</span><strong>{item.teamType==="THREE_X_THREE"?"3x3":"5x5"}</strong><time>{new Date(item.completedAt).toLocaleDateString(language==="th"?"th-TH":"en-GB")}</time><b>{item.points} {dashboardCopy.points}</b><i>→</i></Link></li>)}</ol>:<p className="empty-queue">{copy.noGames}</p>}</section>
    </main>
    <nav className="bottom-nav" aria-label={dashboardCopy.mainNavigation}><Link href="/"><span>⌂</span>{dashboardCopy.home}</Link><Link href="/courts"><span>▱</span>{dashboardCopy.courts}</Link><Link href="/teams"><span>◫</span>{dashboardCopy.teams}</Link><Link href="/maintenance/new"><span>!</span>{dashboardCopy.maintenance}</Link><Link className="is-active" href="/profile"><span>●</span>{dashboardCopy.profile}</Link></nav>
  </div>;
}
