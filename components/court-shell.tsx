...
export function CourtShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="app-frame court-app">
      ...
      <main className="court-shell"><div className="mock-banner" role="note"><span>DEV</span><div><strong>{copy.mockData}</strong><small>{copy.mockLocationHint}</small></div>{process.env.NODE_ENV === "development" ? <button type="button" onClick={resetQueueData}>{copy.resetQueue}</button> : null}</div>{children}</main>
