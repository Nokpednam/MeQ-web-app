...
export function TeamShell({ children, profile }: { children: React.ReactNode; profile: AuthTeamProfile }) {
  ...
  return <div className="app-frame team-app">
    <header className="topbar"><div className="topbar-inner">
      ...
      <nav className="desktop-nav" aria-label={dashboardCopy.mainNavigation}><Link href="/">{dashboardCopy.home}</Link><Link href="/courts">{dashboardCopy.courts}</Link><Link className="nav-current" href="/teams">{dashboardCopy.teams}</Link><Link href="/maintenance/new">{dashboardCopy.maintenance}</Link></nav>
