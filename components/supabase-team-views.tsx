...
function readableError(code: string | null, language: "th" | "en") {
  if (!code) return null;
  const th: Record<string, string> = { TEAM_NAME_LENGTH: "ชื่อทีมต้องมีความยาว 2–30 ตัวอักษร", USER_ALREADY_IN_TEAM: "ผู้ใช้นี้อยู่ในทีมอื่นแล้ว", TEAM_FULL: "สมาชิกทีมครบแล้ว", ROSTER_LOCKED: "ต้องออกจากคิวก่อนจึงจะแก้สมาชิกได้", CAPTAIN_ONLY: "เฉพาะหัวหน้าทีมเท่านั้น", CAPTAIN_CANNOT_LEAVE: "กรุณามอบตำแหน่งหัวหน้าทีมก่อนออก", CAPTAIN_CANNOT_REMOVE_SELF: "หัวหน้าทีมลบตัวเองโดยตรงไม่ได้", USER_NOT_MEMBER: "ไม่พบสมาชิกในทีม", USER_NOT_FOUND: "ไม่พบผู้ใช้นี้", INVITATION_ALREADY_PENDING: "ส่งคำเชิญให้ผู้ใช้นี้แล้ว", INVITATION_NOT_FOUND: "ไม่พบคำเชิญนี้", INVITATION_NOT_PENDING: "คำเชิญนี้ถูกตอบรับหรือยกเลิกแล้ว", CANNOT_INVITE_SELF: "ไม่สามารถเชิญตัวเองได้" };
  const en: Record<string, string> = { TEAM_NAME_LENGTH: "Team name must be 2–30 characters", USER_ALREADY_IN_TEAM: "This user already belongs to a team", TEAM_FULL: "The roster is full", ROSTER_LOCKED: "Leave the queue before editing the roster", CAPTAIN_ONLY: "Only the captain can do this", CAPTAIN_CANNOT_LEAVE: "Transfer captain before leaving", CAPTAIN_CANNOT_REMOVE_SELF: "The captain cannot remove themselves", USER_NOT_MEMBER: "Team member not found", USER_NOT_FOUND: "User not found", INVITATION_ALREADY_PENDING: "An invitation has already been sent to this user", INVITATION_NOT_FOUND: "Invitation not found", INVITATION_NOT_PENDING: "This invitation has already been answered or cancelled", CANNOT_INVITE_SELF: "You cannot invite yourself" };
...
function readableNotice(code: string | null, language: "th" | "en") {
  ...
  const th: Record<string, string> = {
    ...
    "invitation-sent": "ส่งคำเชิญเข้าทีมเรียบร้อยแล้ว",
    "invitation-accepted": "ตอบรับคำเชิญและเข้าทีมเรียบร้อยแล้ว",
    "invitation-declined": "ปฏิเสธคำเชิญเรียบร้อยแล้ว",
    "invitation-cancelled": "ยกเลิกคำเชิญเรียบร้อยแล้ว",
  ...
  const en: Record<string, string> = {
    ...
    "invitation-sent": "The team invitation was sent.",
    "invitation-accepted": "You accepted the invitation and joined the team.",
    "invitation-declined": "The invitation was declined.",
    "invitation-cancelled": "The invitation was cancelled.",
...
export function TeamHomeView({ data }: { data: TeamPageData }) {
  ...
  return <div className="team-page"><header className="team-page-heading"><div><p className="section-label">{copy.pageLabel}</p><h1>{copy.myTeam}</h1></div><Link className="back-link" href="/">← {copy.backDashboard}</Link></header>
    ...
    {data.incomingInvitations.length ? <section className="team-invitation-panel" aria-labelledby="incoming-invitations-title"><div className="invitation-panel-heading"><div><p className="section-label">INVITATIONS</p><h2 id="incoming-invitations-title">{copy.incomingInvitations}</h2><p>{copy.invitationDescription}</p></div><span className="invitation-count">{data.incomingInvitations.length}</span></div><div className="invitation-list">{data.incomingInvitations.map((invitation) => <article className="invitation-card" key={invitation.id}><div><strong>{invitation.teamName}</strong><span>{copy.invitedBy} {invitation.invitedByName}</span></div><div className="invitation-actions"><form action={declineTeamInvitationAction}><input type="hidden" name="invitationId" value={invitation.id}/><TeamSubmitButton className="team-secondary-button" pendingText={language === "th" ? "กำลังปฏิเสธ…" : "Declining…"}>{copy.declineInvitation}</TeamSubmitButton></form><form action={acceptTeamInvitationAction}><input type="hidden" name="invitationId" value={invitation.id}/><TeamSubmitButton className="team-primary-button" disabled={Boolean(team)} pendingText={language === "th" ? "กำลังเข้าทีม…" : "Joining…"}>{copy.acceptInvitation}</TeamSubmitButton></form></div></article>)}</div></section> : null}
...
export function TeamDetailView({ data }: { data: TeamPageData }) {
  ...
  const captain = team.captainUserId === data.currentUser.id; const capacity = getTeamCapacity(team.type); const memberIds = new Set(team.members.map((member) => member.userId)); const invitedUserIds = new Set(data.outgoingInvitations.map((invitation) => invitation.invitedUserId)); const candidates = data.users.filter((user) => !memberIds.has(user.id) && user.displayName.toLowerCase().includes(query.toLowerCase())); const error = readableError(search.get("error"), language); const notice = readableNotice(search.get("notice"), language);
  return <div className="team-page team-detail-page"><header className="team-page-heading"><div><p className="section-label">{copy.pageLabel}</p><h1>{team.name}</h1><p>{copy.fixedType}</p></div><Link className="back-link" href="/teams">← {copy.myTeam}</Link></header>{notice ? <div className="queue-success" role="status">{notice}</div> : null}{error ? <div className="team-error" role="alert">{error}</div> : null}{team.rosterLocked ? <div className="team-queue-notice"><strong>{copy.rosterLocked}</strong><Link href="/courts">{copy.backDashboard} →</Link></div> : null}
    ...
    {dialog ? <div className="dialog-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) setDialog(false); }}><section className="member-dialog" role="dialog" aria-modal="true"><header><h2>{copy.addMember}</h2><button className="dialog-close" onClick={() => setDialog(false)}>×</button></header><label className="search-field"><span>{copy.searchUsers}</span><input value={query} onChange={(e) => setQuery(e.target.value)} autoFocus /></label><div className="candidate-list">{candidates.length ? candidates.map((user) => { const alreadyInvited = invitedUserIds.has(user.id); const unavailable = Boolean(user.currentTeamId) || alreadyInvited; return <article key={user.id}><span className="member-avatar">{user.initials}</span><div><strong>{user.displayName}</strong><small>{user.currentTeamId ? copy.alreadyInOtherTeam : alreadyInvited ? copy.alreadyInvited : copy.availableUsers}</small></div><form action={inviteTeamMemberAction}><input type="hidden" name="teamId" value={team.id}/><input type="hidden" name="userId" value={user.id}/><TeamSubmitButton disabled={unavailable} pendingText={language === "th" ? "กำลังส่ง…" : "Sending…"}>{user.currentTeamId ? copy.alreadyInOtherTeam : alreadyInvited ? copy.alreadyInvited : copy.add}</TeamSubmitButton></form></article>; }) : <p>{copy.noSearchResults}</p>}</div></section></div> : null}
  </div>;
}
