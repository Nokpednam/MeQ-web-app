-- Server-authoritative queue calling, captain readiness, and game creation.
...
begin
  ...
  with ordered as (
    ...
    from public.queue_entries where court_id=p_court_id and status='WAITING'
...
begin
  ...
  update public.queue_entries set status='READY_TO_PLAY' where id=v_session.queue_entry_id;
  ...
  with ordered as (
    select id,row_number() over(order by joined_at,id)::bigint as new_position
    from public.queue_entries where court_id=v_session.court_id and status='WAITING'
  ) update public.queue_entries q set position=ordered.new_position
    from ordered where q.id=ordered.id;
